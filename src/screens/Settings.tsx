...
const SERVICES: { key: StreamingKey; name: string; color: string; signInLabel: string }[] = [
  { key: 'youtube', name: 'YouTube', color: 'var(--youtube)', signInLabel: 'Sign in with Google' },
  { key: 'spotify', name: 'Spotify', color: 'var(--spotify)', signInLabel: 'Connect' },
];
...
export function Settings() {
  ...
  return (
    <div className={styles.page}>
      ...
      {SERVICES.map(({ key, name, color, signInLabel }) => {
        const on = state[key];
          ...
          <div className={styles.card} key={key}>
            <div className={styles.row}>
              ...
              <button className={styles.btn} onClick={onClick} data-tap>
                {busy ? 'Cancel' : on ? 'Sign out' : signInLabel}
              </button>
            </div>
